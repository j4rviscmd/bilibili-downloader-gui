					...
					{hasDlQue &&
						Object.entries(groups).map(([parentId, entries]) => {
							...
							return (
								<div
									key={parentId}
									className="mb-2 w-full rounded-md border px-1.5 py-2 transition-colors duration-200 hover:border-muted-foreground/50"
								>
									{(() => {
										...
										return groupTitle ? (
											<div
												className="text-md mb-0.5 truncate px-2.5 font-semibold leading-tight"
												title={groupTitle}
											>
												{groupTitle}
											</div>
