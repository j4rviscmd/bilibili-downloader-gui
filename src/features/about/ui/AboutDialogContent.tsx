...
export interface AboutDialogContentProps {
  /** Whether the dialog is open. Controlled externally so both the
   * Settings button and the macOS menu-bar "About" item can drive it. */
  open: boolean
  onOpenChange: (open: boolean) => void
}

/**
 * The About dialog body, factored out from the trigger button.
 *
 * Renders the app icon, version, environment info (OS / architecture /
 * Tauri version) and a link to the GitHub repository. Environment info is
 * fetched on first open via the `get_app_info` command. Shared by the
 * Settings-screen trigger (`AboutDialog`) and the macOS menu-bar entry
 * (`MenuAboutDialog`) so both surfaces show the identical information.
 *
 * @example
 * ```tsx
...
export function AboutDialogContent({
  ...
  const handleOpenRepo = useCallback(() => {
    openUrl(REPO_URL).catch((e) =>
      logger.error('AboutDialogContent: failed to open repository', e),
    )
  }, [])
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      logger.error('AboutDialogContent: failed to copy info', e)
