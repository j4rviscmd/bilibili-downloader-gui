...
- **고화질 동영상 다운로드** - 8K/4K/1080p/720p/HDR10/돌비 비전 등 원하는 화질 선택 가능
- **Bangumi(애니메이션·시리즈) 지원** - 일반 동영상과 함께 애니메이션 및 시리즈 에피소드 다운로드 가능
- **멀티파트 동영상 일괄 저장** - 강좌, 드라마 등 모든 파트 자동 다운로드
- **고속 안정 다운로드** - CDN 자동 전환, 네트워크 오류 시 자동 재시도
- **다운로드 대기열** - 다운로드 중에도 검색과 추가 가능. 직렬 대기열(FIFO)이 다운로드 페이지에서 처리되며 하단 바에 진행률 표시
- **자막 임베드** - 소프트/하드 자막 선택, 다국어 및 AI 자막 지원
- **하이레즈 오디오** - Dolby Atmos, Hi-Res Lossless 지원

### 로컬 MP4 도구

- **트림** - 시작/종료 시간을 지정해 로컬 MP4 자르기(무손실 복사 또는 재인코딩)
- **병합** - 여러 MP4 파일을 하나로 합치기(코덱 불일치 시 자동 재인코딩)
- **오디오 추출** - 로컬 MP4에서 MP3/M4A로 오디오 추출, 비트레이트 선택 가능
- **GIF / WebM 생성** - 로컬 MP4 클립에서 애니메이션 GIF(256색) 또는 무음 WebM 생성(너비/FPS 프리셋)

### Bilibili 연동

- **즐겨찾기** - Bilibili 즐겨찾기 폴더를 탐색하고 다운로드
- **시청 기록** - Bilibili 시청 기록에서 바로 다운로드
- **단축 URL 자동 확장** - b23.tv 단축 URL을 전체 동영상 URL로 자동 확장

### 사용 편의성

- **6개국어 UI** - 영어 / 일본어 / 프랑스어 / 스페인어 / 중국어 / 한국어
- **원클릭 설정** - FFmpeg 자동 설치(기능 검증 포함), 수동 설정 불필요
- **자동 업데이트** - 서명 검증 업데이터 및 릴리스 노트 내장
- **기록 검색 및 내보내기** - 다운로드 기록을 JSON/CSV로 내보내기
- **다크 모드 지원** - 라이트/다크 테마 전환

### 인증 방법

- **Firefox 쿠키 자동 감지** - Firefox 브라우저의 쿠키를 감지하여 수동 로그인 없이 고화질 다운로드 가능
- **QR 코드 로그인** - 앱 내에서 QR 코드를 스캔하여 로그인
- **수동 쿠키 붙여넣기** - 브라우저 DevTools에서 복사한 `Cookie` 헤더를 붙여넣으면 QR 세션과 동일하게 검증 후 저장됩니다
  - 쿠키, QR, 수동 로그인은 언제든지 전환 가능
...
> [!NOTE]
> Linux 빌드는 v1.62.0부터 Ubuntu 24.04에서 컴파일되며 glibc 2.39 이상(Debian 13+, Ubuntu 24.04+)이 필요합니다. Ubuntu 22.04, Debian 12 등 이전 배포판에서는 v1.62.0 이후 버전이 `GLIBC_2.39 not found` 오류로 시작되지 않으므로 해당 시스템에서는 업데이트하지 마세요.
