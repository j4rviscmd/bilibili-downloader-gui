...
- **高品質動画ダウンロード** - 8K/4K/1080p/720p/HDR10/Dolby Visionから任意の画質を選択可能
- **バングミ（アニメ・シリーズ）対応** - 通常動画に加えて、アニメやシリーズの各話をダウンロード可能
- **マルチパート動画一括保存** - 講座、シリーズなどすべてのパートを自動ダウンロード
- **高速・安定ダウンロード** - CDN自動切り替え、ネットワークエラー時の自動リトライ
- **ダウンロードキュー** - ダウンロード中も検索・キュー追加が可能。直列キュー（FIFO）をダウンロードページで消化し、ボトムバーに進捗を表示
- **字幕埋め込み** - ソフト/ハード字幕選択、多言語・AI字幕対応
- **ハイレゾ音声** - Dolby Atmosおよびハイレゾロスレス音質サポート

### ローカルMP4ツール

- **トリム** - 開始/終了時刻を指定してローカルMP4をカット（ロスレスコピーまたは再エンコード）
- **結合** - 複数のMP4ファイルを1つに結合（コーデック不一致時は自動で再エンコード）
- **音声抽出** - ローカルMP4からMP3/M4Aで音声を抽出、ビットレート選択可能
- **GIF / WebM 生成** - ローカルMP4のクリップからアニメーションGIF（256色）または無音WebMを生成（幅/FPSプリセット）

### Bilibili連携

- **お気に入り** - Bilibiliのお気に入りフォルダを参照してダウンロード
- **視聴履歴** - Bilibiliの視聴履歴から直接ダウンロード
- **短縮URL自動展開** - b23.tvの短縮URLを自動的にフルURLへ展開

### 使いやすさ

- **6言語UI** - 英語 / 日本語 / フランス語 / スペイン語 / 中国語 / 韓国語
- **ワンクリックセットアップ** - FFmpeg自動インストール（機能検証付き）、手動設定不要
- **自動アップデート** - 署名検証付きアップデーターとリリースノートを内蔵
- **履歴検索・エクスポート** - ダウンロード履歴をJSON/CSVにエクスポート
- **ダークモード対応** - ライト/ダークテーマ切り替え

### 認証方法

- **Firefox Cookie自動検出** - FirefoxのCookieを検出し、手動ログインなしで高品質ダウンロード
- **QRコードログイン** - アプリ内でQRコードをスキャンしてログイン
- **手動Cookie貼り付け** - ブラウザのDevToolsからコピーした`Cookie`ヘッダーを貼り付け。QRセッションと同様に検証して保存
  - Cookie・QR・手動ログインはいつでも切り替え可能

### プライバシーとセキュリティ

- **安全な認証情報管理** - QRコードログインの認証情報はAES-256-GCMで暗号化してローカルに保存。Argon2idによる鍵導出でマシン固有の保護を実現
- **ローカルのみ保存** - ダウンロードした動画はPC内にのみ保存
- **トラッキングなし** - Bilibili APIとGitHub（アップデート確認用）とのみ通信、テレメトリなし
